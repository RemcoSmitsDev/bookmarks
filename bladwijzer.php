...
<nav id="nav" class="relative py-6 px-4 w-full sm:flex sm:items-center sm:justify-between container mx-auto max-w-screen-xl z-auto" >
        <a class="inline-flex space-x-1 items-center text-3xl font-semibold transform hover:scale-110 text-white outline-none" href="./">
    ...
    <div class="mt-4 md:mt-0 flex space-x-4">
        <?php if(count($cats) > 0):  ?>
            <div>
                <button id="showItemMenu" class="px-4 py-2 inline-flex items-center rounded bg-white bg-opacity-25 text-white font-semibold transform hover:scale-110 outline-none">Create bookmark
                    <span class="mt-px">
        ...
        <?php endif; ?>
            <button id="showCatMenu" class="px-4 py-2 inline-flex items-center rounded bg-white bg-opacity-25 text-white font-semibold transform hover:scale-110 outline-none">Create category
                <span class="mt-px">
                    <svg class="h-6 w-6 mt-px" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                    </svg>
        ...
        <div class="relative hidden md:inline-block">
            <span class="rounded-md shadow-sm" id="show-updates">
                <button type="button" class="inline-flex justify-center w-full rounded-md px-4 py-2 bg-white bg-opacity-25 text-white hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                ...
                <div class="rounded-md bg-white shadow-xs h-64 overflow-y-auto remove-scrollbar">
                    <div class="py-1 divide-y">
                        <?php foreach ($updates as $update): ?>
                            <span class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 first:text-gray-900"><?= $update->message; ?></span>
                        <?php endforeach; ?>
                    </div>
                </div>
            </div>
...
<div id="create-form" class="flex items-center h-screen opacity-0 pointer-events-none fixed inset-0 bg-black bg-opacity-50 transition-opacity ease-in duration-150 z-10">
    <form id="fromCR"
        ...
        <input class="px-4 py-2 border-2 focus:outline-none rounded appearance-none hidden" id="cat" type="text" placeholder="Categorie naam" value="">
        <input class="px-4 py-2 border-2 focus:outline-none rounded appearance-none" id="title" type="text" placeholder="Name" value="">
        <input class="px-4 py-2 border-2 focus:outline-none rounded appearance-none" id="url" type="text" placeholder="https://www.google.com/" value="">
        <select class="px-4 py-2 border-2 focus:outline-none rounded appearance-none" id="cat_id">
            <?php foreach ($cats as $cat) : ?>
                <option value="<?= $cat->id ?>"><?= $cat->name ?></option>
            <?php endforeach; ?>
        </select>
        <button id="add" class="px-4 py-2 bg-blue-500 rounded font-semibold text-white focus:outline-none">Create new bookmark</button>
    </form>
</div>
<div id="editMenu" class="flex items-center h-screen opacity-0 pointer-events-none fixed inset-0 bg-black bg-opacity-50 transition-opacity ease-in duration-150 z-10">
    <form id="editForm"
        ...
        <button id="saveEdit" class="px-4 py-2 bg-blue-500 rounded font-semibold text-white focus:outline-none">Save changes</button>
    </form>
</div>
            ...
            <div id="item" class="opacity-0">
                ...
                <div class="space-y-2 flex flex-col">
                    <?php foreach ($bladwijzers->GetItemsByCat($cat->id) as $item) : ?>
                        <span id="<?= $item->id; ?>" class="group inline-flex items-center space-x-4"><a target="_blank" class="flex items-center underline hover:no-underline focus:outline-none" href="<?= $item->url; ?>"><div class="h-6 w-6 bg-no-repeat object-contain bg-center" style='background-image: url(https://www.google.com/s2/favicons?domain=<?= $item->url ?>);'></div><span><?= $item->title; ?></span></a>
