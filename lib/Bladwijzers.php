...
class Bladwijzers
{
    ...
    public function getCategoriesForWebhook($user_id)
    {
    ...
    public function insertNewWithWebhook($title, $url, $cat_id, $user_id)
    {
        $new_title = $title;
        if ($this->checkIfitemAlreadyExist($url, $user_id)) {
            return false;
        }
        if (strlen($title) > 50) {
    ...
    public function checkIfitemAlreadyExist($url, $user_id)
    {
        $this->db->query("SELECT * FROM items WHERE url = :url AND user_id = :user_id");
        $this->db->bind(":url", $url);
        $this->db->bind(":user_id", $user_id);
