...
<head>
    ...
    <script data-ad-client="ca-pub-1783394865391064" async
...
<body class="antialiased overflow-x-hidden">
    ...
    <nav id="nav"
        class="relative py-6 px-4 w-full flex items-center justify-between container mx-auto max-w-screen-xl z-auto">
        <a class="inline-flex space-x-1 items-center text-3xl font-semibold transform hover:scale-110 text-white"
            href="./"><svg class="h-10 w-10" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
        ...
        <div class="hidden md:block">
            ...
            <div class="space-x-4">
                <a class="px-4 py-2 inline-flex items-center rounded bg-white bg-opacity-25 text-white font-semibold transform hover:scale-110 focus:outline-none"
                    href="login">Login</a>
                <a class="px-4 py-2 inline-flex items-center rounded border border-bg-green-400 text-white font-semibold transform hover:scale-110 focus:outline-none"
                    href="register">Create an account</a>
            </div>
            <?php else: ?>
            <a class="group px-4 py-2 inline-flex items-center rounded bg-white bg-opacity-25 text-white font-semibold transform hover:scale-110 focus:outline-none"
                href="<?= $_SESSION['_user']->first_name. "-".$_SESSION['_user']->last_name ?>">Go to your bookmarks
                <span class="mt-px">
        ...
        <button id="toggleMenu"
            class="h-8 w-8 inline-flex items-center justify-center md:hidden rounded bg-white bg-opacity-25 focus:outline-none">
            <svg class="flex-shrink-0 h-6 w-6 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                fill="currentColor">
                <path fill-rule="evenodd"
    ...
    <div id="menu" class="hidden opacity-0 transition-all duration-1000 p-4 fixed inset-x-0 inset-y-0 z-20">
        <div class="p-4 bg-white h-full w-full rounded shadow-lg">
            ...
            <div class="mt-20 space-x-6 flex items-center justify-center">
                <?php if(!isset($_SESSION['_user'])):  ?>
                <a class="px-4 py-2 inline-flex items-center rounded text-white font-semibold transform hover:scale-110 focus:outline-none"
                    href="login" style="background: linear-gradient(90deg, #0072ff 0%, #00d4ff 100%);">Login</a>
                <a class="px-4 py-2 inline-flex items-center rounded border border-bg-green-400 font-semibold transform hover:scale-110 focus:outline-none"
                    href="register">Create an account</a>
                <?php else: ?>
                <a class="px-4 py-2 inline-flex items-center rounded text-white font-semibold transform hover:scale-110 focus:outline-none"
                    href="<?= $_SESSION['_user']->first_name. "-".$_SESSION['_user']->last_name ?>"
                    style="background: linear-gradient(90deg, #0072ff 0%, #00d4ff 100%);">Go to your bookmarks
                    <span class="mt-px">
                        <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd"
                                d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"
    ...
    <section id="banner"
        class="mx-4 mt-16 md:mt-32 xl:space-x-20 space-y-16 xl:space-y-0 xl:flex-row flex-col flex items-center justify-center z-10">
        <div id="text" class="opacity-0">
            <h1 class="block leading-tight font-bold text-2xl sm:text-5xl text-gray-900">All your <span
                    class="leading-tight font-bold text-2xl sm:text-5xl text-5xl sm:text-6xl md:text-6xl text-gray-900">bookmarks</span>
            </h1>
